...
* Highly available [Alertmanager](https://github.com/nholuongut/alertmanager)
* [Prometheus node-exporter](https://github.com/nholuongut/node-exporter)
* [Prometheus Adapter for Kubernetes Metrics APIs](https://github.com/nholuongut/prometheus-adapter)
* [kube-state-metrics](https://github.com/nholuongut/kube-state-metrics)
...
This stack provides [resource metrics](https://github.com/nholuongut/metrics#resource-metrics-api) by deploying the [Prometheus Adapter](https://github.com/nholuongut/prometheus-adapter/).
...
To try out this stack, start [minikube](https://github.com/nholuongut/minikube) with the following command:
...
Install this library in your own project with [jsonnet-bundler](https://github.com/nholuongut/jsonnet-bundler#install) (the jsonnet package manager):
```shell
$ mkdir my-kube-prometheus; cd my-kube-prometheus
$ jb init  # Creates the initial/empty `jsonnetfile.json`
# Install the kube-prometheus dependency
$ jb install github.com/nholuongut/kube-prometheus/jsonnet/kube-prometheus@release-0.4 # Creates `vendor/` & `jsonnetfile.lock.json`, and fills in `jsonnetfile.json`
```

> `jb` can be installed with `go get github.com/nholuongut/jsonnet-bundler/cmd/jb`

> An e.g. of how to install a given version of this library: `jb install github.com/nholuongut/kube-prometheus/jsonnet/kube-prometheus@release-0.4`
...
```shell
$ go get -u github.com/nholuongut/jsonnet-bundler/cmd/jb
```
...
```
{
    ...
    imageRepos+:: {
        ...
        alertmanager: "quay.io/nholuongut/alertmanager",
...
The grafana definition is located in a different project (https://github.com/nholuongut/kubernetes-grafana), but needed configuration can be customized from the same top level `_config` field. For example to allow anonymous access to grafana, add the following `_config` section:
```
      grafana+:: {
...
```jsonnet
(import 'nholuongut/kube-prometheus.libsonnet') +
(import 'nholuongut/kube-prometheus-bootkube.libsonnet')
```
...
```jsonnet
(import 'nholuongut/kube-prometheus.libsonnet') +
(import 'nholuongut/kube-prometheus-kops.libsonnet')
```
...
(import 'nholuongut/kube-prometheus.libsonnet') +
(import 'nholuongut/kube-prometheus-kops.libsonnet') +
(import 'nholuongut/kube-prometheus-kops-coredns.libsonnet')
```
...
```jsonnet
(import 'nholuongut/kube-prometheus.libsonnet') +
(import 'nholuongut/kube-prometheus-kubespray.libsonnet')
```
...
```jsonnet
(import 'nholuongut/kube-prometheus.libsonnet') +
(import 'nholuongut/kube-prometheus-kube-aws.libsonnet')
```
...
$ docker pull quay.io/nholuongut/alertmanager:v0.16.2
$ docker tag quay.io/nholuongut/alertmanager:v0.16.2 internal-registry.com/organization/alertmanager:v0.16.2
...
```jsonnet
local mixin = import 'nholuongut/kube-prometheus-config-mixins.libsonnet';
...
Standard Kubernetes manifests are all written using [ksonnet-lib](https://github.com/nholuongut/ksonnet-lib/), so they can be modified with the mixins supplied by ksonnet-lib. For example to override the namespace of the node-exporter DaemonSet:
...
```jsonnet
local kp = (import 'nholuongut/kube-prometheus.libsonnet') +
           (import 'nholuongut/kube-prometheus-all-namespaces.libsonnet') + {
  _config+:: {
    namespace: 'monitoring',
...
possible, one can include the [kube-prometheus-anti-affinity.libsonnet](jsonnet/nholuongut/kube-prometheus-anti-affinity.libsonnet) mixin:
...
```jsonnet
local kp = (import 'nholuongut/kube-prometheus.libsonnet') +
           (import 'nholuongut/kube-prometheus-strip-limits.libsonnet') + {
  _config+:: {
    namespace: 'monitoring',
  },
};
...
See [exposing nholuongut/alertmanager/Grafana](docs/exposing-prometheus-alertmanager-grafana-ingress.md) guide.
...
[addon-resizer](https://github.com/nholuongut/autoscaler/tree/master/addon-resizer/nanny)
